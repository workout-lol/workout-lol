...
;(async () => {
  ...
  const category = new CollectContent(
    'table[title="Exercise Matrix"] tbody tr:nth-child(1) td:last-child',
    { name: 'category' }
  )
  const difficulty = new CollectContent(
    'table[title="Exercise Matrix"] tbody tr:nth-child(2) td:last-child',
    { name: 'difficulty' }
  )
  const targets = new CollectContent(
    'table[title="Muscles  Targeted"] tbody tr td:last-child',
    { name: 'targets' }
  )
  const video = new DownloadContent('.exercise-images-grid video', {
    name: 'video',
    filePath: './videos',
  })
